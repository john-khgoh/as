...
estimator = XGBRegressor(n_estimators=100,eval_metric=mean_squared_error)
...
summed_y_error = 0.0
summed_xy_error = 0.0
len_y_test = len(y_test_df)
for i in range(len_y_test):
    x_diff = abs(y_test_df['x'][i] - yhat_test_df['x'][i]) #Difference of x-coordinates between y_hat and y
    y_diff = abs(y_test_df['y'][i] - yhat_test_df['y'][i]) #Difference of y-coordinates between y_hat and y
    summed_x_error += x_diff
    summed_y_error += y_diff
    summed_xy_error += ((x_diff + y_diff) / 2.0) #Getting the average between the two
mean_x_error = summed_x_error / len_y_test
mean_y_error = summed_y_error / len_y_test
mean_xy_error = summed_xy_error / len_y_test
